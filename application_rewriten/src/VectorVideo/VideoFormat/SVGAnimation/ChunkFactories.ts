	...
	export class ChunkFactory {
		...
		protected InitCommandsFromSVG(node: Element, cmdFactory: CommandFactory): Array<Command> {
			if (node.localName === this.InitCommandsNodeName) {
				...
				var cmd: Element = node.firstElementChild;
				while (!!cmd) {
		...
		protected InitCommandsToSVG(cmds: Array<Command>, cmdFactory: CommandFactory): Node {
			var node: Node = SVGA.CreateElement(this.InitCommandsNodeName);
	...
	export class VoidChunkFactory extends ChunkFactory {
		...
		FromSVG(node: Element, cmdFactory: CommandFactory): Chunk {
			if (SVGA.attr(node, "type") === VoidChunkFactory.NodeName) {
				var chunk: VoidChunk = new VoidChunk(SVGA.numAttr(node, "t"), SVGA.numAttr(node, "lastErase"));
				// load init commands
				var init: Element = node.firstElementChild;
				chunk.InitCommands = this.InitCommandsFromSVG(init, cmdFactory);
				
				var cmd: Element = init.nextElementSibling;
				while (!!cmd) {
	...
	export class PathChunkFactory extends ChunkFactory {
		...
		FromSVG(node: Element, cmdFactory: CommandFactory): Chunk {
			if (SVGA.attr(node, "type") === PathChunkFactory.NodeName) {				
				...
				var init: Element = pathNode.nextElementSibling;
	...
	export class EraseChunkFactory extends ChunkFactory {
		...
		FromSVG(node: Element, cmdFactory: CommandFactory): Chunk {
			if (SVGA.attr(node, "type") === EraseChunkFactory.NodeName) {				
				...
				// [2] second node must be init commands container
				var init: Element = rectNode.nextElementSibling;
				if (init.localName !== this.InitCommandsNodeName) {
					throw new Error(`Second child of chunk must be an <init> element, but ${init.localName} found instead`);
				}
				var initCmds: Array<Command> = this.InitCommandsFromSVG(init, cmdFactory);
