...
(function (Drawing) {
    ...
    var SvgPath = (function () {
        ...
        SvgPath.prototype.StartPath = function (position, radius) {
            // init SVG
            this.startDot = SVG.CreateDot(position, radius, this.color);
            this.path = SVG.CreateElement("path", { "fill": this.color });
            // prepare paths
        ...
        SvgPath.prototype.ConnectWithCanvas = function (canvas) {
            this.canvas = canvas;
        };
        SvgPath.prototype.ExtendPath = function (right, left) {
            this.right += SVG.LineToString(right);
            this.left = SVG.LineToString(this.lastLeft) + " " + this.left;
            // A] - a simple line at the end of the line 
            //this.cap = SVG.LineToString(left);
            // B] - an "arc cap"
            var center = right.add(left).scale(0.5);
            var startDirection = right.subtract(center);
            var endDirection = left.subtract(center);
            this.cap = SVG.ArcString(left, center.distanceTo(left), this.angle(startDirection));
            this.lastLeft = left;
        };
        SvgPath.prototype.angle = function (vec) {
    ...
    var CanvasPath = (function () {
        /** Init empty path */
        function CanvasPath(color) {
            this.color = color;
            this.points = [];
        }
        Object.defineProperty(CanvasPath.prototype, "LastPoint", {
        ...
        CanvasPath.prototype.StartPath = function (position, radius) {
            this.points.push({
                left: position,
                right: position
            });
        ...
        CanvasPath.prototype.ConnectWithCanvas = function (context) {
        ...
        CanvasPath.prototype.ExtendPath = function (right, left) {
            this.DrawSegment(this.LastPoint.right, this.LastPoint.left, right, left);
            this.points.push({
                left: left,
                right: right
            });
        };
        CanvasPath.prototype.DrawSegment = function (lastRight, lastLeft, right, left) {
            this.context.beginPath();
            this.context.moveTo(lastRight.X, lastRight.Y);
            this.context.lineTo(lastLeft.X, lastLeft.Y);
            ...
            var startDirection = right.subtract(center);
            var endDirection = left.subtract(center);
            this.context.arc(center.X, center.Y, center.distanceTo(left), this.angle(startDirection), this.angle(endDirection), false);
            this.context.closePath();
            this.context.fillStyle = this.color;
            this.context.fill();
        };
        CanvasPath.prototype.angle = function (vec) {
            return Math.atan2(-vec.X, vec.Y) - Math.PI / 2; /// :-) 
        };
        /**
         * Promote the curve to the DOM
        ...
        CanvasPath.prototype.Redraw = function () {
            // now draw the start dot
            this.drawDot(this.startPosition, this.startRadius);
            for (var i = 1; i < this.points.length; i++) {
                var prev = this.points[i - 1];
                var next = this.points[i];
                this.DrawSegment(prev.right, prev.left, next.right, next.left);
            }
            // and the end dot
            this.drawDot(this.LastPoint.left.add(this.LastPoint.right).scale(0.5), this.LastPoint.left.distanceTo(this.LastPoint.right) / 2);
        };
        return CanvasPath;
    })();
...
(function (Drawing) {
    ...
    var BrushInstance = (function () {
        function BrushInstance(mass, drag, size) {
            this.mass = mass;
            this.drag = drag;
            this.size = size;
        }
        ...
        Object.defineProperty(BrushInstance.prototype, "Drag", {
    ...
    var Cursor = (function () {
         ...
         */
        Cursor.prototype.Apply = function (destination) {
            // calculate the force
            var force = destination.subtract(this.position);
            ...
            this.velocity = this.velocity.scale(1 - this.brush.Drag); // drag is the analogy of friction
            // update position
        ...
        Cursor.prototype.Draw = function (path, pressure) {
            ...
            var relativeSpeed = this.velocity.getSize() / (this.brush.Size * this.brush.Size);
            var width = this.getRadius(pressure, relativeSpeed);
            var delta = this.angle.scale(width);
            if (this.firstSegment) {
                path.ExtendPath(this.startPosition.add(delta), this.startPosition.subtract(delta));
        ...
        Cursor.prototype.StartPath = function (path, pt, pressure) {
            //var width: number = this.getRadius(pressure);
            path.StartPath(pt, this.getRadius(pressure, 0));
        };
        Cursor.prototype.getRadius = function (pressure, speed) {
            return this.speedFactor(speed) * this.brush.Size * pressure / 2;
        };
    ...
    var DynaDraw = (function () {
        ...
        DynaDraw.GetBrush = function (brushSize) {
            if (!this.brushes[brushSize]) {
                var mass = this.MassFromSize(brushSize);
                console.log("Size: ", brushSize, "Mass: ", mass);
                this.brushes[brushSize] = new BrushInstance(mass, 0.4, brushSize);
            }
            return this.brushes[brushSize];
        };
        /**
         * Linear interpolation between min and max weight based on brush size
         * @param	{number}	size	Brush size
         * @return	{number}			Brush mass
         */
        DynaDraw.MassFromSize = function (size) {
            if (size >= this.maxSize)
                return this.maxMass;
            else if (size <= this.minSize)
                return this.minSize;
            return this.minMass + (this.maxMass - this.minMass) * (size - this.minSize) / (this.maxSize - this.minSize);
        };
        /**
        ...
        DynaDraw.prototype.StartPath = function (position, pressure, brushSize, path) {
            this.cursor.Reset(position, DynaDraw.GetBrush(brushSize));
            this.position = position;
            this.pressure = pressure;
            this.path = path;
         ...
         *
         */
        DynaDraw.prototype.Tick = function () {
            var _this = this;
            if (!!this.position) {
                //while(this.cursor.Apply(this.position)) {
                //} // -- while is interesting, but doesn't give as good results
                if (this.cursor.Apply(this.position)) {
            ...
            // do the next tick
            requestAnimationFrame(function () { return _this.Tick(); }); // ~ 60 FPS
...
(function (Drawing) {
    ...
    var SVGDrawer = (function () {
        function SVGDrawer() {
            ...
            this.canvasWrapper.GetHTML().appendChild(svg);
            ...
            svg.addEventListener("DOMNodeInsertedIntoDocument", function (e) {
                // this is event handler - "this" isn't SVGDrawer here! 
                var parent = svg.parentElement;
                var width = parent.clientWidth;
                var height = parent.clientHeight;
                Helpers.HTML.SetAttributes(e.target, {
                    width: width,
                    height: height
                });
                Helpers.SVG.SetAttributes(bg, {
                    width: width,
                    height: height
                });
            ...
            // init DynaDraw
            this.dynaDraw = new Drawing.DynaDraw();
        }
        /**
        ...
        SVGDrawer.prototype.StartLine = function (point, pressure) {
            var path = new Drawing.SvgPath(this.settings.Color);
            path.ConnectWithCanvas(this.canvas);
        ...
        SVGDrawer.prototype.ContinueLine = function (nextPoint, pressure) {
            this.dynaDraw.NextPoint(nextPoint, pressure);
        };
        /**
        ...
        SVGDrawer.prototype.EndLine = function (position) {
            this.dynaDraw.EndPath(position, 0);
        };
        return SVGDrawer;
    })();
...
(function (UI) {
    ...
    var Cursor = (function () {
        ...
        Cursor.prototype.CreateHTML = function () {
            this.element = Helpers.HTML.CreateElement("div");
        ...
        Cursor.prototype.ChangeSize = function (size) {
            ...
            var calculatedSize = Math.max(size.Size, this.stroke * 2 + 1);
...
(function (VectorVideo) {
    var Player = (function () {
        function Player(id, settings) {
            ...
            this.drawer = new Drawing.SVGDrawer();
...
(function (VideoData) {
    ...
    var Mouse = (function () {
        function Mouse(board) {
            ...
            document.onblur = function (e) { return _this.onLooseFocus(e); }; // mouse up can't be detected when window (tab) is not focused - the app behaviour would be weird
            // video events			
        ...
        Mouse.prototype.GetPressure = function () {
            return (this.isMouseDown === true && this.isMouseInside === true) ? 1 : 0;
        };
        /**
        ...
        Mouse.prototype.onMouseLeave = function (e) {
            if (this.isRunning && this.GetPressure() > 0) {
                this.onMouseMove(e); // draw one more segment
                this.isMouseDown = false;
                this.onMouseMove(e); // discontinue the line
                this.isMouseDown = true; // back to current state
            }
        ...
        Mouse.prototype.onMouseEnter = function (e) {
            if (e.buttons === 0) {
                this.isMouseDown = false; // check mouse down status
            }
        };
        /**
    ...
    var WacomTablet = (function (_super) {
        ...
        WacomTablet.prototype.GetPressure = function () {
            if (this.penApi && this.penApi.pointerType == WacomPointerType.Pen) {
                return this.isMouseInside === true ? this.penApi.pressure : 0; // pressure is from 0.0 (no pressure) to 1.0 (max pressure)
            }
        };
        /**
...
(function (Drawing) {
    ...
    var CanvasDrawer = (function () {
        function CanvasDrawer() {
            ...
            // stretch the canvas as much as possible as soon as it knows where it is in the DOM
            // see http://www.w3.org/TR/2003/NOTE-DOM-Level-3-Events-20031107/DOM3-Events.html#events-event-DOMNodeInsertedIntoDocument
            var $this = this;
            this.canvas.addEventListener("DOMNodeInsertedIntoDocument", function (e) { return _this.Stretch(e); });
            // settings will be set later
            this.settings = {
                Color: "",
                Size: 0
        ...
        CanvasDrawer.prototype.Stretch = function (e) {
            // this is event handler - "this" isn't SVGDrawer here!
            var canvas = e.target;
            var parent = canvas.parentElement;
            var width = parent.clientWidth;
            var height = parent.clientHeight;
            Helpers.HTML.SetAttributes(canvas, {
                width: width,
                height: height
            });
         ...
         * @param   size    The new size of the brush (line thickness)
         */
        CanvasDrawer.prototype.SetBrushSize = function (size) {
            this.settings.Size = size.Size;
        };
        /**
         * Set current brush color
         * @param   color   The new color of the brush
         */
        CanvasDrawer.prototype.SetBrushColor = function (color) {
            this.settings.Color = color.CssValue;
        };
        /**
         * Process next state and
         */
        CanvasDrawer.prototype.ProcessNewState = function (cursor) {
            try {
                var nextPoint = new Vector2(cursor.X, cursor.Y);
                if (cursor.Pressure > 0) {
                    if (!this.lastState || this.lastState.Pressure === 0) {
        ...
        CanvasDrawer.prototype.ClearCanvas = function () {
            this.context.fillStyle = UI.Color.BackgroundColor;
        ...
        CanvasDrawer.prototype.StartLine = function (point, pressure) {
            var path = new Drawing.CanvasPath(this.settings.Color);
            path.ConnectWithCanvas(this.context);
            this.dynaDraw.StartPath(point, pressure, this.settings.Size, path);
        };
        /**
         * Prolong the current line.
         ...
         * @param   pressure    The pressure of the pointing device in this point.
         */
        CanvasDrawer.prototype.ContinueLine = function (nextPoint, pressure) {
            this.dynaDraw.NextPoint(nextPoint, pressure);
        };
        /**
         * Current brush settings
         * @return {object} Settings.
         */
        CanvasDrawer.prototype.GetCurrentBrushSettings = function () {
            return this.settings;
        };
        /**
         * Ends currently drawn line.
         * @param {Vector2} pos Position of mouse when pressure dropped to 0.
         */
        CanvasDrawer.prototype.EndLine = function (position) {
            this.dynaDraw.EndPath(position, 0);
        };
        /**
         ...
         */
        CanvasDrawer.prototype.RedrawEverything = function () {
            this.ClearCanvas();
    ...
    var RecorderUI = (function (_super) {
        ...
        RecorderUI.prototype.RecordPause = function () {
            if (this.isRecording === true) {
                ...
                this.board.GetHTML().classList.remove("recording");
            }
            else {
                this.StartRecording();
                Helpers.HTML.SetAttributes(this.uploadButton.GetHTML(), { "disabled": "disabled" });
                this.board.GetHTML().classList.add("recording");
            }
        };
        /**
         * Start (or continue) recording
        ...
        RecorderUI.prototype.Tick = function () {
            ...
            this.recPauseButton.ChangeContent(Helpers.millisecondsToString(this.recordingTime));
        };
    ...
    var Recorder = (function () {
        ...
        function Recorder(id, settings) {
            ...
            if (!settings.ColorPallete || settings.ColorPallete.length === 0) {
                ...
                colors.push(new UI.Color("eraser", UI.Color.BackgroundColor.CssValue));
